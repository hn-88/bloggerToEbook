// This code is based on the code available here: https://github.com/hn-88/bloggerToEbook which is licensed under 
// "MIT License" .."Copyright (c) 2022 hn-88".
...
//var blogurl = 'https://ravisiyermisc.blogspot.com/';
//var desiredebooktitle = 'Test BlogBook'; //Ravi: Added Test to file name
var contenthtml = '';
const DEF_BLOG_URL = 'https://ravisiyermisc.blogspot.com/';
const BOOK_TITLE = 'BlogBook'; 
const DEF_MAX_POSTS_PER_PART = 10;  // Test run small default value
const DEF_MAX_TOTAL_POSTS = 20;     // Test run small default value

function myFunction(blogurlarg, yr, maxpostsperpart, maxtotalposts, fetchurlmainpart) {
  Logger.log("myFunction arguments: blogurlarg = " + blogurlarg + ", yr = " + yr + ", maxpostsperpart = " + maxpostsperpart + ', maxtotalposts = ' + maxtotalposts + ', fetchurlmainpart = ' + fetchurlmainpart);
  ...
  if (fetchurlmainpart == null)
  {
    ...
    if (yr == null)
      ...
      var prevyr = +yr - 1;
      var nextyr = +yr + 1;
      apicall = blogurl+'feeds/posts/default'+'?max-results='+ maxresults + '&published-min=' + prevyr +'-12-31T00:00:00-08:00&published-max='+ nextyr +'-01-01T23:59:59-08:00&alt=json'; 
      // Timezone +05:30 trips up the program. I think the + character needs to be escaped but don't know how.
      // So as a temp fix I am adding a day before min and a day after max
    }
  }
  else {
      ...
      Logger.log ("After checking for max-results string in fetchurlmainpart, maxresults variable set to: " + maxresults);
  ...
  do { 
    ...
    for ( i in json.feed.entry) {
      ...
      contenthtml+='<h1>'+json.feed.entry[i].title.$t
            ...
            +'<br/>===========================End of Post============================<br/>'
            + '<span style="break-after: always;" />'; //Ravi: Does not create page break in Google Docs document
      Logger.log("Post Title (80 chars), pubdate of this fetch: '" + json.feed.entry[i].title.$t.substring(0,80) + "', " + json.feed.entry[i].published.$t) ;
      Logger.log('%s in this fetch, %s overall',(+i+1), (+postindex+(+i)));
    
    }
    postindex = +postindex+(+i)+1; //Ravi: postindex is used for start_index which seems to be a 1 based index
                                    ...
                                    // the right value for start_index for next call to fetch to get the next set
                                    // of maxresults posts.

    try {
      var ablob = Utilities.newBlob(contenthtml, MimeType.HTML, "asset.html");
      var AssetGDocId = Drive.Files.insert(
        { title: BOOK_TITLE+' part '+ loopindex, 
        mimeType: MimeType.GOOGLE_DOCS },
        ablob
      ).id;
      Logger.log('Wrote "%s" to GDoc.', BOOK_TITLE+' part '+ loopindex );
    }
    catch(err){
  ...
  return ("Seems to be successful run. See Execution Log for run details.<br/>");
}

function doGet(e) {
  var blogurlparm = e.parameter["blogurl"];
  var year = e.parameter["year"];
  var maxpostsperpart = e.parameter["maxpostsperpart"];
  var maxtotalposts = e.parameter["maxtotalposts"];
  var fetchurlmainpart = e.parameter["fetchurlmainpart"];

  Logger.log("blogurl parameter = "+ e.parameter["blogurl"]);
  Logger.log("year parameter = "+ e.parameter["year"]);
  Logger.log("maxpostsperpart parameter = "+ e.parameter["maxpostsperpart"]);
  Logger.log("maxtotalposts parameter = "+ e.parameter["maxtotalposts"]);
  Logger.log("fetchurlmainpart parameter = "+ e.parameter["fetchurlmainpart"]);

  var Finmsg = myFunction(blogurlparm, year, maxpostsperpart, maxtotalposts, fetchurlmainpart);
  return HtmlService.createHtmlOutput(Finmsg);
}
...
if (indexOfampchar == -1) {
  ...
  if (indexOfspacechar == -1) {
    ...
    console.log("No further parameters and after truncating part from first space character, maxresultsstr = '" + maxresultsstr + "'");
  }
}
else {
